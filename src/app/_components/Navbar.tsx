...
export default function Navbar({ links }: { links: CustomLinkType[] }) {
  ...
  const hideMenuTrigger = useCallback(() => {
    if (window.innerWidth > 1024) {
      setHideMenu(false);
    }
  }, []);
  useViewportSize(hideMenuTrigger);
  return (
    <nav className="relative bg-black py-2 px-10 w-full xl:w-[80%] mx-auto sticky top-0 xl:top-5 xl:rounded-lg flex flex-col lg:flex-row lg:justify-between z-1">
      ...
      <ol
        className={clsx(
          "list-none flex flex-col lg:flex-row justify-center py-10 gap-6 lg:gap-10 2xl:gap-20 lg:py-0",
          {
            hidden: hideMenu,
          },
        )}
      >
        {links.map((link) => (
          <li className="my-auto text-nowrap" key={link.href}>
