...
const SUBTITLE_SIZES: Partial<
  Record<"sm" | "md" | "lg" | "xl" | "xxl", string>
> = {
  ...
  return (
    ...
    >
      <Image
        ...
        width={chosenLogoSize}
        height={chosenLogoSize}
        src={"/imgs/pitagorici-logo.png"}
        alt="Pitagorici APS Logo"
      />
      <div className={clsx({ "my-auto": direction === "horizontal" })}>
        <h2 className={TITLE_SIZES[size]}>Pitagorici APS</h2>
        {size !== "sm" && (
          <p className={SUBTITLE_SIZES[size] || "text-lg"}>
            Errare humanum est, perseverare... <b>pythagoricum!</b>
          </p>
        )}
      </div>
    </div>
